...
public class SaveLoad : MonoBehaviour {
	...
	void Start () {
		saveData = new Save();  // Save is a serial class below
		...
		input_brokerInfo.onValueChanged.AddListener(delegate {Input_brokerInfo_ChangeCheck(input_brokerInfo.text); });
	...
	public void Input_brokerInfo_ChangeCheck(string newText)
    {
		saveData.brokerInfo = newText;
		//Convert to Json
		string jsonData = JsonUtility.ToJson(saveData);
		//Save Json string
		PlayerPrefs.SetString("MySettings", jsonData);
	...
	void LoadData(){
		...
		input_brokerInfo.text = jsonData.brokerInfo;
...
public class Save
{
	...
	public String[] hat_ids = new String[6];
}